...
pub async fn setup_server() {
    let home = warp::get().map(|| String::from("Hello and welcome to outof"));

    let lists = warp::path::param()
        .and(warp::path("lists"))
        .map(|id: String| build_lists_page(&id))
    ...
    let routes = warp::get().and(lists.or(delete).or(list).or(home));

    println!(
        "Starting server on {}...",
        std::env::var("DOMAIN").unwrap_or_else(|_| "http://127.0.0.1:8888/".to_string())
    );
